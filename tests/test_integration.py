...
async def test_full_graph_workflow(client):
    ...
    try:
        ...
        analysis_response = client.post("/api/graph/analyze-content", json={
        ...
        logger.error(f"Full graph workflow test failed: {str(e)}")
        raise
...
async def test_error_recovery(client):
    ...
    try:
        ...
        error_response = client.post("/api/graph/analyze-content", json={
            "invalid": "data"
        })
...
async def test_websocket_updates(client):
    ...
    try:
        ...
        logger.error(f"WebSocket updates test failed: {str(e)}")
        raise
